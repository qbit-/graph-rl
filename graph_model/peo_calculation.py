...
def get_upper_bound_peo_pace2017(
        old_graph, method="tamaki", wait_time=60, with_time=False):
    """
    ...
    method_args = {
        'tamaki':
        {'command': 'java -cp .  tw.heuristic.MainDecomposer',
         'wait_time': wait_time,
    ...
    if with_time:
        tree, treewidth, result_time = read_td_file(out_data, as_data=True, with_time=True)
    else:
    ...
    peo = [inv_dict[pp] for pp in peo]
    return peo, treewidth, result_time
...
def get_upper_bound_peo(graph, method='tamaki', **kwargs):
    ...
    **kwargs: default {}
    ...
    _time = 0
    if method in pace_heuristics: # put ifo about time
